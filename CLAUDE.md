...
## Dynamic WebP serving (v0.3.9)
Bug found in real use: conversion rewrote stored content only; featured images / theme output / render-time srcset are generated from attachment metadata (still jpg) every request → visitors kept getting originals. Fix: serve twins dynamically at output edges. CRITICAL ORDERING LESSON (#23): swapping src in `wp_get_attachment_image_src` makes core `wp_calculate_image_srcset` bail (basename mismatch, returns false BEFORE its filter fires) → responsive images die. Correct architecture: let core compute srcset from original filenames, then swap at final-HTML stage — filters `wp_get_attachment_image` (final <img> html) + `wp_content_img_tag` (content render) + `wp_calculate_image_srcset` (entry URLs), all priority 9 (before Replacer's ?v= at 10). Per-URL twin-existence check so partial conversions degrade gracefully. Verified: featured image emits webp src + full webp srcset + sizes.
